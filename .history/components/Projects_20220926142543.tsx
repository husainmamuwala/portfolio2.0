...
const Projects: React.FC = () =>{
  const projects = [
    ...
    {
      name:'T.Alimahomed & Co.',
      type:'Full Stack',
      desc:'lorem ipsum',
    },
    {
      name:'Ogun Hosting',
      type:'Front End UI/UX',
      desc:'lorem ipsum',
    }
  ];
  return (
    <div className='h-screen relative flex overflow-hidden flex-col text-left md:flex-row 
        ...
        <div className='relative w-full flex overflow-hidden  overflow-x-scroll  snap-x snap-mandatory z-20'>
           {projects.map((p,index) => (
            ...
            <div key={in} className='w-screen flex-shrink-0 snap-center items-center flex flex-col justify-center
             space-y-5 md:p-44 xl:p-60 h-screen'>
                ...
                <div className="space-y-10 px-0 md:px-10 max-w-6xl">
                    <h4 className='text-l text-center font-semibold'> 
                      <span className='underline decoration-[#1f9aec] text-3xl'>{p.name}</span>- {p.type}
                    </h4>
                      <p className='max-w-lg text-center md:max-w-3xl p-2 md:p-0'>{p.desc}</p>
                </div>
            </div>
  
           ))}
        </div>
        <div className='w-full absolute top-[30%] bg-[#1f9aec]/10 left-0 h-[500px] -skew-y-12'></div>   
    </div>
  )
  }
